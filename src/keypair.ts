import * as crypto from 'crypto';
import { readFile, writeFile } from 'fs/promises';

export type KeyPairAlgorithm =
	'RS256' | 'RS384' | 'RS512' | 'ES256' | 'ES384' | 'ES512';

export interface KeyPair {
	publicKey: crypto.KeyObject;
	privateKey: crypto.KeyObject;
	algorithm: KeyPairAlgorithm;
}

export class KeyPairGenerator {
	public algorithm: KeyPairAlgorithm;

	public constructor(options: { algorithm: KeyPairAlgorithm }) {
		this.algorithm = options.algorithm;
	}

	public generate(): KeyPair {
		let type: 'rsa' | 'ec' | null = null;
		let keyOptions: crypto.AsymmetricKeyDetails = {};

		if (this.algorithm === 'RS256') {
			type = 'rsa';
			keyOptions.modulusLength = 2048;
		}
		else if (this.algorithm === 'RS384') {
			type = 'rsa';
			keyOptions.modulusLength = 3072;
		}
		else if (this.algorithm === 'RS512') {
			type = 'rsa';
			keyOptions.modulusLength = 4096;
		}
		else if (this.algorithm === 'ES256') {
			type = 'ec';
			keyOptions.namedCurve = 'prime256v1';
		}
		else if (this.algorithm === 'ES384') {
			type = 'ec';
			keyOptions.namedCurve = 'secp384r1';
		}
		else if (this.algorithm === 'ES512') {
			type = 'ec';
			keyOptions.namedCurve = 'secp521r1';
		}

		if (type === null) {
			throw new Error('Keypair type could not be inferred');
		}

		const { publicKey, privateKey } = crypto.generateKeyPairSync(
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			type as any,
			keyOptions
		);

		return {
			algorithm: this.algorithm,
			publicKey,
			privateKey,
		};
	}

	public async load(options: {
		publicKeyPath: string;
		privateKeyPath: string;
	}): Promise<KeyPair> {
		const publicKey = (await readFile(options.publicKeyPath)).toString();
		const privateKey = (await readFile(options.privateKeyPath)).toString();

		return {
			algorithm: this.algorithm,
			publicKey: crypto.createPublicKey(publicKey),
			privateKey: crypto.createPrivateKey(privateKey),
		};
	}

	public async save(options: {
		keys: KeyPair;
		publicKeyPath: string;
		privateKeyPath: string;
	}): Promise<void> {
		const publicKey = options.keys.publicKey;
		const privateKey = options.keys.privateKey;

		await writeFile(
			options.publicKeyPath,
			publicKey.export({ format: 'pem', type: 'spki' })
		);
		await writeFile(
			options.privateKeyPath,
			privateKey.export({ format: 'pem', type: 'pkcs8' })
		);
	}
}
